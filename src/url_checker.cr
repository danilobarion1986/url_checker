...
WORKERS = 2
url_result_stream = Channel(String).new # Channel, with the type it receives
result_stream = Channel({String, Int32 | Exception}).new
stats_stream = Channel(Array({String, Stats::UrlInfo})).new

UrlReader.run("./urls.yml", url_result_stream)
WORKERS.times {
  StatusChecker.run(url_result_stream, result_stream)
}
